...
function workspaceTitle(workspace: OrcaWorkspace): string {
	return workspace.title || humanize(workspace.directoryName);
}
				...
				<span className="block text-[11px] tracking-[0.04em] text-muted-foreground">
					{allImported
						...
						: repo.alreadyImportedCount > 0
							? `${repo.alreadyImportedCount}/${repo.workspaceCount} added`
...
function ImportWorkspaceRow({
	...
	const checkboxId = `settings-import-orca-workspace-${workspace.id}`;
	...
	return (
		...
		>
			<Checkbox
				...
				aria-label={`Select ${workspaceTitle(workspace)}`}
			/>
			<div className="min-w-0 flex-1">
				<span className="block truncate text-[13px] font-medium text-foreground">
					{workspaceTitle(workspace)}
				...
				<div className="flex items-center gap-2 text-[11px] tracking-[0.04em] text-muted-foreground">
					...
					<span>{workspace.status ?? "in-progress"}</span>
				</div>
			</div>
		</label>
	);
...
export function OrcaImportPanel() {
	const queryClient = useQueryClient();
	const searchRef = useRef<HTMLInputElement>(null);

	const [repos, setRepos] = useState<OrcaRepo[]>([]);
	const [workspaces, setWorkspaces] = useState<OrcaWorkspace[]>([]);
	...
	useEffect(() => {
		...
		listOrcaWorkspaces(selectedRepoId)
			.then((ws) => {
				setWorkspaces(ws);
				setSelectedIds(
					new Set(ws.filter((w) => !w.alreadyImported).map((w) => w.id)),
				);
			})
			.catch(() => setWorkspaces([]))
	...
	const filteredWorkspaces = useMemo(() => {
		...
		return workspaces.filter((w) => {
			const haystack =
				`${w.directoryName} ${w.branch ?? ""} ${w.title ?? ""} ${w.absolutePath}`.toLowerCase();
	...
	const handleImport = useCallback(async () => {
		...
		setImportSuccess(null);
		try {
			const result = await importOrcaWorkspaces(Array.from(selectedIds));
			if (result.importedCount > 0) {
				invalidateAfterImport();
			}
			if (result.errors.length > 0) {
				setImportError(
					`${result.importedCount} added, ${result.errors.length} failed: ${result.errors[0]}`,
				);
			} else {
				setImportSuccess(
					`Added ${result.importedCount} workspace${result.importedCount === 1 ? "" : "s"}`,
				);
				reloadWorkspaces();
			}
		} catch (e) {
	...
	}, [importing, invalidateAfterImport, reloadWorkspaces, selectedIds]);
		...
		<>
			<SettingsGroup>
				<SettingsRow
					...
					description={
						selectedRepoId
							? "Select Orca workspaces to add"
							: "Add Orca workspaces without moving their directories"
					}
				/>
			</SettingsGroup>
			...
			<div className="mt-3">
				{importing ? (
					<div className="flex flex-col items-center justify-center gap-3 py-8">
						...
						<div className="text-center">
							<p className="text-[13px] font-medium text-foreground">
								Adding {selectedIds.size} workspace
								{selectedIds.size === 1 ? "" : "s"}
							</p>
							<p className="mt-1 text-[11px] text-muted-foreground">
								Linking Orca directories into Helmor...
							</p>
						</div>
					</div>
				) : loadingRepos ? (
					...
					<Empty className="py-6">
						<EmptyHeader>
							<EmptyTitle>
								{repos.length === 0 ? "No Orca workspaces found" : "No matches"}
							</EmptyTitle>
						</EmptyHeader>
					</Empty>
				)}
				...
				<div className="mt-4">
					...
					>
						<FolderInput
							...
							strokeWidth={1.8}
						/>
						Add {selectedIds.size} workspace{selectedIds.size === 1 ? "" : "s"}
					</Button>
				</div>
			)}
		</>
