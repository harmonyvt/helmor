...
export function projectSidebarListsByGoal(
	...
	const allRows = withPending.flatMap((group) => group.rows);
	...
	for (const [goalId, goalRow] of goalRowsById) {
		goalGroups.push({
			...
			goalRow,
			childRows: childrenByGoalId.get(goalId) ?? [],
		});
	...
	goalGroups.sort((a, b) =>
		(b.goalRow.createdAt ?? "").localeCompare(a.goalRow.createdAt ?? ""),
	);
	...
	const projectGroups: GoalProjectGroup[] = Array.from(
		byRepo.entries(),
		([
			repoName,
...
export function projectSidebarListsByPr(
	...
	return projectGroups.sort((a, b) => a.repoName.localeCompare(b.repoName));
}
