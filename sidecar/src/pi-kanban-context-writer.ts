  ...
  pi.on("before_agent_start", async (event, ctx) => {
    try {
      ...
      contextLines.push(
		...
		"Use list_project_workspaces(include_archived?) to inspect every workspace in this project/repository, including standalone Claude/Codex/Pi workspaces outside the current goal board. Then use list_threads(workspace_id), create_thread(workspace_id), get_thread(workspace_id, thread_id), update_thread(workspace_id, thread_id, title), delete_thread(workspace_id, thread_id), and send_thread_message(workspace_id, thread_id, message) to inspect and manage threads inside a workspace.",
		...
		"Use knowledge tools when context should persist beyond the current thread: search_knowledge(query, scope?), get_knowledge_status(), reindex_knowledge(scope?), query_project_knowledge(query), query_goal_knowledge(query), and record_goal_knowledge_note(title?, text, metadata?). Use them for no-code cards, decisions, and lifecycle facts that child workspaces should inherit.",
        ...
        "Do not move cards into merged. Helmor derives the merged lane from whether the child workspace has landed in the goal branch.",
      ...
      for (const lane of laneOrder) {
        ...
        for (const c of laneCards) {
          ...
          const prTag = c.prUrl
            ? c.prSyncState === "merged"
              ...
              : c.prSyncState === "closed"
                ...
                : " [PR open]"
            : "";
          const landingTag = c.landingState === "landed"
            ? \` [landed\${c.landingSource ? \` via \${c.landingSource}\` : ""}]\`
            : "";
          ...
          contextLines.push(\`- [workspace:\${c.id}] \${c.title}\${branchTag}\${prTag}\${landingTag}\${threadsTag}\${assigneeTag}\${activeStatusTag}\`);
