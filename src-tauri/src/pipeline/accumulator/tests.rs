...
#[test]
fn codex_item_start_reserves_turn_order_for_late_completion() {
    let mut acc = StreamAccumulator::new("codex", "gpt-5.4");

    let cmd_started = json!({
        "type": "item/started",
        "itemId": "cmd-slow",
        "item": {"type": "commandExecution", "id": "cmd-slow", "command": "sleep 10"}
    });
    acc.push_event(&cmd_started, &cmd_started.to_string());

    let read_started = json!({
        "type": "item/started",
        "itemId": "pi-read",
        "item": {
            "type": "mcpToolCall",
    ...
    acc.push_event(&read_started, &read_started.to_string());

    let read_completed = json!({
        "type": "item/completed",
        "itemId": "pi-read",
        "item": {
            "type": "mcpToolCall",
    ...
    acc.push_event(&read_completed, &read_completed.to_string());

    let cmd_completed = json!({
        "type": "item/completed",
        "itemId": "cmd-slow",
        "item": {
            "type": "commandExecution",
    ...
    acc.push_event(&cmd_completed, &cmd_completed.to_string());

    assert_eq!(acc.turn_at(0).id, "codex-cmd-asst:cmd-slow");
    assert_eq!(acc.turn_at(1).id, "codex-mcp-asst:pi-read");
    assert_eq!(acc.turn_at(2).id, "codex-mcp-asst:pi-read");
    assert_eq!(acc.turn_at(3).id, "codex-cmd-asst:cmd-slow");
}

#[test]
fn codex_pi_mcp_tool_call_extracts_text_result() {
    let mut acc = StreamAccumulator::new("codex", "gpt-5.4");
    ...
    let asst = snapshot
        ...
        .unwrap();
    let block = &asst.parsed.as_ref().unwrap()["message"]["content"][0];
    assert_eq!(block["name"].as_str(), Some("mcp__pi__read"));
    assert_eq!(block["input"]["path"].as_str(), Some("src/App.tsx"));

    let user = snapshot
        .iter()
        .find(|m| m.role == MessageRole::User)
        .unwrap();
    let result = &user.parsed.as_ref().unwrap()["message"]["content"][0];
    assert_eq!(result["content"].as_str(), Some("file contents"));
}
