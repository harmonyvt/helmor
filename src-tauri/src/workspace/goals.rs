...
pub fn prepare_goal_workspace(
    ...
    let existing_pr = source_branch
        .as_deref()
        .map(|branch| resolve_github_pull_request_by_head_branch(&repo_root, branch))
        .transpose()?
        .flatten();
    let title = existing_pr
        .as_ref()
        .and_then(|pr| normalize_optional_str(&pr.title))
        .or(title)
        .context("Goal title is required")?;
    let description = existing_pr
        .as_ref()
        .and_then(|pr| normalize_optional_str(pr.body.as_deref().unwrap_or("")))
        .or(description)
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
struct GoalSourcePullRequest {
    title: String,
    body: Option<String>,
}

fn resolve_github_pull_request_by_head_branch(
    repo_root: &Path,
    branch: &str,
) -> Result<Option<GoalSourcePullRequest>> {
    ...
    if !output.status.success() {
        ...
        bail!("Failed to look up pull request for branch {branch}: {detail}");
    }
    let pulls: Vec<GoalSourcePullRequest> =
