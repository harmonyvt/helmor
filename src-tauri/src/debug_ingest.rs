...
use ngrok_tunnel::{close_public_tunnel, NgrokTunnelManager, PublicTunnelHandle};
...
pub struct DebugIngestManager {
    ...
    ngrok_tunnels: NgrokTunnelManager,
}
...
impl Default for DebugIngestManager {
    fn default() -> Self {
        Self {
            ...
            ngrok_tunnels: NgrokTunnelManager::default(),
        }
    }
}
...
impl DebugIngestManager {
    ...
    async fn ensure_public_tunnel(
        ...
        let mut tunnel = Some(self.ngrok_tunnels.start(addr, config).await?);
        {
            let mut servers = self.servers.lock().unwrap_or_else(|e| e.into_inner());
    ...
    pub async fn overview(&self) -> DebugIngestOverview {
        ...
        let active_tunnel_count = instances
            ...
            .count();
        DebugIngestOverview {
            ngrok_agent: self.ngrok_tunnels.status(active_tunnel_count).await,
            instances,
        }
    }
    ...
    fn close_ngrok_agent_if_idle(&self) {
        ...
        if !has_public_tunnel {
            self.ngrok_tunnels.close_agent_if_idle();
        }
    }
...
fn public_tunnel_config_key(config: &DebugIngestPublicForwardConfig) -> String {
    ngrok_tunnel::config_key(config)
}
