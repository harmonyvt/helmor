...
fn unsupported_embedded_browser(action: &str, tab_id: &str) -> anyhow::Error {
    let label = browser_webview_label(tab_id);
    anyhow!(
        "Browser {action} is unavailable in this build: embedded child webviews require Tauri's unstable child-webview API (label: {label})."
    )
}
...
pub async fn create_browser_webview(
    ...
    Err(anyhow!(
        "Embedded browser webviews are unavailable in this build: Helmor is built without Tauri's unstable child-webview API."
    )
    .into())
}

#[tauri::command]
pub async fn browser_go_back(_app: AppHandle, tab_id: String) -> CmdResult<()> {
    Err(unsupported_embedded_browser("back navigation", &tab_id).into())
}

#[tauri::command]
pub async fn browser_go_forward(_app: AppHandle, tab_id: String) -> CmdResult<()> {
    Err(unsupported_embedded_browser("forward navigation", &tab_id).into())
}

#[tauri::command]
pub async fn open_browser_devtools(_app: AppHandle, tab_id: String) -> CmdResult<()> {
    Err(unsupported_embedded_browser("devtools", &tab_id).into())
}
