...
pub const WORKSPACE_RECORD_SQL: &str = r#"
    ...
    SELECT
      ...
      r.remote,
      r.forge_provider,
      COALESCE(w.location_kind, 'local') AS location_kind,
      w.remote_profile_id,
      w.remote_backend,
      w.remote_root_path,
      w.remote_container_name,
      w.remote_host,
      w.remote_status,
      w.remote_error,
      w.created_at,
...
pub(crate) struct InitializingWorkspaceMetadata<'a> {
    ...
    pub(crate) location_kind: Option<&'a str>,
    pub(crate) remote_profile_id: Option<&'a str>,
    pub(crate) remote_backend: Option<&'a str>,
    pub(crate) remote_root_path: Option<&'a str>,
    pub(crate) remote_container_name: Option<&'a str>,
    pub(crate) remote_host: Option<&'a str>,
    pub(crate) remote_status: Option<&'a str>,
    pub(crate) remote_error: Option<&'a str>,
    ...
    transaction
        .execute(
            ...
            rusqlite::params![
                ...
                metadata.pr_url,
                metadata.location_kind.unwrap_or("local"),
                metadata.remote_profile_id,
                metadata.remote_backend,
                metadata.remote_root_path,
                metadata.remote_container_name,
                metadata.remote_host,
                metadata.remote_status,
                metadata.remote_error,
                metadata.timestamp,
            ],
        )
...
fn workspace_record_from_row(row: &Row<'_>) -> rusqlite::Result<WorkspaceRecord> {
    Ok(WorkspaceRecord {
        ...
        location_kind: row.get(31)?,
        remote_profile_id: row.get(32)?,
        remote_backend: row.get(33)?,
        remote_root_path: row.get(34)?,
        remote_container_name: row.get(35)?,
        remote_host: row.get(36)?,
        remote_status: row.get(37)?,
        remote_error: row.get(38)?,
        created_at: row.get(39)?,
        updated_at: row.get(40)?,
        last_user_message_at: row.get(41)?,
    })
}
